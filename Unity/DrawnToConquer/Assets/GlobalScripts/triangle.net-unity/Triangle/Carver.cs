...
namespace TriangleNet
{
    ...
    class Carver
    {
        ...
        public Carver(TriangleNetMesh mesh)
        {
            this.mesh = mesh;
        ...
        private void InfectHull()
        {
            ...
            do
            {
                ...
                if (!hulltri.IsInfected())
                {
                    ...
                    if (hullsubseg.seg == TriangleNetMesh.dummysub)
                    {
        ...
        void Plague()
        {
            ...
            for (int i = 0; i < viri.Count; i++)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    if ((neighbor.triangle == TriangleNetMesh.dummytri) || neighbor.IsInfected())
                    {
                        if (neighborsubseg.seg != TriangleNetMesh.dummysub)
                        {
                            // There is a subsegment separating the triangle from its
                            // neighbor, but both triangles are dying, so the subsegment
                            // dies too.
                            mesh.SubsegDealloc(neighborsubseg.seg);
                            if (neighbor.triangle != TriangleNetMesh.dummytri)
                            {
            ...
            foreach (var virus in viri)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    if (testvertex != null)
                    {
                        ...
                        while ((neighbor.triangle != TriangleNetMesh.dummytri) &&
        ...
        public void CarveHoles()
        {
            ...
            if (!mesh.behavior.NoHoles)
            {
                ...
                foreach (var hole in mesh.holes)
                {
                    ...
                    if (mesh.bounds.Contains(hole))
                    {
                        ...
                        searchtri.triangle = TriangleNetMesh.dummytri;
                        searchtri.orient = 0;
                        searchtri.SymSelf();
            ...
            if (mesh.regions.Count > 0)
            {
                ...
                {
                    regionTris[i] = TriangleNetMesh.dummytri;
            ...
            if (regionTris != null)
            {
                ...
                {
                    if (regionTris[i] != TriangleNetMesh.dummytri)
                    {
