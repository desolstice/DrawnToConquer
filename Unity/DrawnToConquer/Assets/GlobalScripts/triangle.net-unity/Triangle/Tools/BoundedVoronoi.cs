...
namespace TriangleNet.Tools
{
    ...
    public class BoundedVoronoi : IVoronoi
    {
        ...
        public BoundedVoronoi(TriangleNetMesh mesh)
            : this(mesh, true)
        {
        }
        ...
        /// </summary>
        /// <param name="mesh">Mesh instance.</param>
        public BoundedVoronoi(TriangleNetMesh mesh, bool includeBoundary)
        {
            this.mesh = mesh;
        ...
        private void TagBlindTriangles()
        {
            ...
            foreach (var ss in mesh.subsegs.Values)
            {
                ...
                if (f.triangle != TriangleNetMesh.dummytri && !f.triangle.infected)
                {
                ...
                while (triangles.Count > 0)
                {
                    ...
                    if (TriangleIsBlinded(ref f, ref e))
                    {
                        ...
                        for (f.orient = 0; f.orient < 3; f.orient++)
                        {
                            ...
                            if (f0.triangle != TriangleNetMesh.dummytri && !f0.triangle.infected && sub1.seg == TriangleNetMesh.dummysub)
                            {
        ...
        private void ConstructBoundaryBvdCell(Vertex vertex)
        {
            ...
            if (f_prev.triangle != TriangleNetMesh.dummytri)
            {
                // Go clockwise until we reach the border (or the initial triangle)
                while (f_prev.triangle != TriangleNetMesh.dummytri && !f_prev.Equal(f_init))
                {
                    f_prev.Copy(ref f);
                    f_prev.OprevSelf();
                }
            ...
            do
            {
                ...
                if (f_next.triangle == TriangleNetMesh.dummytri)
                {
                    if (!f.triangle.infected)
                    {
