...
namespace TriangleNet
{
    ...
    class Quality
    {
        ...
        public Quality(TriangleNetMesh mesh)
        {
        ...
        public bool CheckMesh()
        {
            ...
            foreach (var t in mesh.triangles.Values)
            {
                ...
                for (tri.orient = 0; tri.orient < 3; tri.orient++)
                {
                    ...
                    tri.Sym(ref oppotri);
                    if (oppotri.triangle != TriangleNetMesh.dummytri)
                    {
        ...
        public bool CheckDelaunay()
        {
            ...
            foreach (var tri in mesh.triangles.Values)
            {
                     ...
                     loop.orient++)
                {
                    ...
                    shouldbedelaunay = (oppotri.triangle != TriangleNetMesh.dummytri) &&
                    ...
                    if (mesh.checksegments && shouldbedelaunay)
                    {
                        ...
                        if (opposubseg.seg != TriangleNetMesh.dummysub)
                        {
        ...
        public int CheckSeg4Encroach(ref Osub testsubseg)
        {
            ...
            if (neighbortri.triangle != TriangleNetMesh.dummytri)
            {
                sides++;
        ...
        public void TestTriangle(ref Otri testtri)
        {
            ...
            if ((angle > behavior.goodAngle) || (maxangle < behavior.maxGoodAngle && behavior.MaxAngle != 0.0))
            {
                    ...
                    (base2.type == VertexType.SegmentVertex))
                {
                    ...
                    if (testsub.seg == TriangleNetMesh.dummysub)
                    {
                        ...
                        while (testsub.seg == TriangleNetMesh.dummysub);
        ...
        private void SplitEncSegs(bool triflaws)
        {
            ...
            while (badsubsegs.Count > 0)
            {
                ...
                if (!Osub.IsDead(currentenc.seg) && (eorg == seg.subsegorg) && (edest == seg.subsegdest))
                {
                    ...
                    acuteorg = testsh.seg != TriangleNetMesh.dummysub;
                    // Is the destination shared with another segment?
                    testtri.LnextSelf();
                    testtri.SegPivot(ref testsh);
                    acutedest = testsh.seg != TriangleNetMesh.dummysub;
                    ...
                    if (testtri.triangle != TriangleNetMesh.dummytri)
                    {
                        // Is the destination shared with another segment?
                        testtri.LnextSelf();
                        testtri.SegPivot(ref testsh);
                        acutedest2 = testsh.seg != TriangleNetMesh.dummysub;
                        acutedest = acutedest || acutedest2;
                        // Is the origin shared with another segment?
                        testtri.LnextSelf();
                        testtri.SegPivot(ref testsh);
                        acuteorg2 = testsh.seg != TriangleNetMesh.dummysub;
